...
const sHopping = ref('fa-solid fa-cart-shopping');
const Order = ref('fa-solid fa-cash-register');
const Add = ref('fa-solid fa-cart-plus')
const Delete = ref('fa-solid fa-trash-can')
const Cart = ref(0);
...
<div class="menu d-flex justify-content-between m-5">
    <h3 class="mx-5"><div class="col-4 pt-1">
        <RouterLink class="nav-item nav-link link-body-emphasis active" to="/">
          <img src="https://amarinacademy.com/app/uploads/2017/06/06_bottlekitchen.jpg" alt="Logo" class="logo-img" style="max-height: 60px;"> 
        </RouterLink>
        <!--ใส่รูปโลโกแทนคำว่า Home ทำให้กดที่โลโกกลับไปที่หน้าหลัก-->
      </div></h3>
    ...
    <div class="row row-cols-1 row-cols-md-4 g-4">
      <div class="col" v-for="(item, index) in shop_food" :key="index">
        <div class="card">
          ...
          <button @click="addCart(item.name, item.img, item.price)" class="btn btn-success"><i :class="Add"></i></button>
        </div>
      </div>
    </div>
  </div>
...
.menu {
  ...
  align-items: center;
  padding: 20px;
  background: linear-gradient(to right, #ffffff, #1d8963);
  color: #000000;
  border-radius: 20px;
}
